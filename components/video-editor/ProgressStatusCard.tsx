...
export function ProgressStatusCard({ job }: { job: VideoEditorJob | null }) {
  const step = getPipelineStep(job?.currentStep);
  const progress = job?.progress ?? 0;
  const label =
    job?.currentStepLabel ||
    step?.label ||
    (job ? job.currentStep : "Consultando job");
    ...
    <section className="rounded-[8px] border border-white/10 bg-white/[0.07] p-5 shadow-[0_34px_120px_-70px_rgba(0,0,0,1)] backdrop-blur-xl sm:p-7">
      <div className="grid gap-6 lg:grid-cols-[auto_1fr] lg:items-center">
        <div className="rounded-[8px] border border-[#dfc18a]/25 bg-black/20 p-5 sm:min-w-56">
          <p className="text-sm font-medium text-zinc-300">Estado del job</p>
          <p className="mt-3 text-2xl font-semibold leading-tight text-white">
            {job?.status || "cargando"}
          </p>
          <p className="mt-3 text-sm leading-6 text-zinc-400">
            {step?.description || "Leyendo el JSON de progreso local."}
          </p>
        </div>
